...
\begin{lstlisting}
% Rearrange: y_dot = (f(t) - 5*sin(y)) / 3
% Assume f(t) = 1 (unit step input)
f = 1;
ode = @(t, y) (f - 5*sin(y)) / 3;
[t, y] = ode45(ode, [0, 10], 0);

plot(t, y);
title('Solution of 3\dot{y} + 5 sin y = f(t)');
xlabel('t'); ylabel('y(t)');
grid on;
\end{lstlisting}
...
\[\dot{x} = -3x + 2y + f(t), \qquad \dot{y} = 4x - 5y\]

\begin{lstlisting}
% State vector: s(1) = x, s(2) = y
% Assume f(t) = 1 (unit step)
f = @(t) 1;
ode = @(t, s) [-3*s(1) + 2*s(2) + f(t);
                4*s(1) - 5*s(2)];
[t, sol] = ode45(ode, [0, 5], [0; 0]);

plot(t, sol(:,1), '-', t, sol(:,2), '--', 'LineWidth', 1.5);
legend('x(t)', 'y(t)');
title('State System Simulation');
xlabel('t'); grid on;
\end{lstlisting}
...
% y_ddot = (7*sin(4t) + 5*cos(3t)) / 10
ode = @(t, x) [x(2);
               (7*sin(4*t) + 5*cos(3*t)) / 10];
[t, x] = ode45(ode, [0, 6], [3; 2]);

plot(t, x(:,1), 'LineWidth', 1.5);
title('Solution of 10\ddot{y} = 7sin4t + 5cos3t');
xlabel('t'); ylabel('y(t)');
grid on;
\end{lstlisting}
...
\begin{lstlisting}
% Numerical: x_dot = tan(t) - x
% Note: tan(t) diverges near t = pi/2 ~= 1.57; limit range
ode = @(t, x) tan(t) - x;
[t, x_num] = ode45(ode, [0, 1.4], 0);

% Approximate solution
x_approx = (1/3)*t.^3 - t.^2 + 3*t - 3 + 3*exp(-t);

plot(t, x_num, 'b-', t, x_approx, 'r--', 'LineWidth', 1.5);
legend('Numerical (ode45)', 'Approximate');
title('Comparison: \dot{x} + x = tan t');
xlabel('t'); ylabel('x(t)');
grid on;
\end{lstlisting}
...
\[\dot{x}_1 = -6x_1 + 4x_2, \qquad \dot{x}_2 = 5x_1 - 7x_2 + f(t)\]
